...
import { TAX_RATE_REDUCED, TAX_RATE_STANDARD, taxFromInclusive } from '../util/money.js';
...
export function buildInvoiceView(db: DB, orderId: number): InvoiceView | null {
  ...
  const buyerName = invoice?.buyer_name ?? getCustomerName(db, order.customer_id);
  const items = listInvoiceItems(db, order.id);
  const taxRateBreakdown = buildTaxRateBreakdown(items);
